...
export class GeneralLedgerController {
  ...
  @ApiOperation({ summary: 'Get general ledger entries with filtering and pagination' })
  ...
  @ApiResponse({ status: 200, description: 'General ledger entries retrieved successfully' })
  async getGeneralLedgerEntries(
    @CurrentUser() user: any,
    @Query() query: GeneralLedgerQueryDto
  ) {
    try {
      const result = await this.generalLedgerService.getGeneralLedgerEntries(user.organizationId, query);
  ...
  @Roles(UserRole.ADMIN, UserRole.TENANT_ADMIN, UserRole.MANAGER, UserRole.USER, UserRole.VIEWER)
  @ApiOperation({ summary: 'Get account ledger (all entries for a specific account)' })
  @ApiQuery({ name: 'dateFrom', required: false, type: Date })
  @ApiQuery({ name: 'dateTo', required: false, type: Date })
  ...
  async generateIncomeStatement(
    ...
    try {
      ...
      const incomeStatement = await this.generalLedgerService.generateIncomeStatement(
        user.organizationId,
        periodFromParsed,
        periodToParsed
      );
  ...
  @ApiResponse({ status: 200, description: 'General ledger integrity validation completed' })
  async validateGeneralLedgerIntegrity(@CurrentUser() user: any) {
    try {
      const validation = await this.generalLedgerService.validateGeneralLedgerIntegrity(user.organizationId);
