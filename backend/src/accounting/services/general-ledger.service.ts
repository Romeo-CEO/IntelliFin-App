...
export class GeneralLedgerService {
  ...
  async getAccountLedger(
    ...
    try {
      ...
      return await this.generalLedgerRepository.getAccountLedger(
        organizationId,
        accountId,
        dateFrom,
        dateTo
  ...
  async getAccountBalance(
    ...
    try {
      ...
      return await this.generalLedgerRepository.getAccountBalance(
        organizationId,
        accountId,
        asOfDate
      );
  ...
  async generateTrialBalance(
    ...
    try {
      this.logger.log(
        `Generating trial balance for organization: ${organizationId} as of ${asOfDate.toISOString()}`
      );

      const trialBalance =
        await this.generalLedgerRepository.generateTrialBalance(
          organizationId,
          asOfDate
        );

      this.logger.log(
        `Generated trial balance with ${trialBalance.accounts.length} accounts. Balanced: ${trialBalance.isBalanced}`
      );
      return trialBalance;
    } catch (error) {
      this.logger.error(
  ...
  async generateBalanceSheet(
    ...
    try {
      this.logger.log(
        `Generating balance sheet for organization: ${organizationId} as of ${asOfDate.toISOString()}`
      );

      // Get account balances by type
      const [assetBalances, liabilityBalances, equityBalances] =
        await Promise.all([
          this.generalLedgerRepository.getAccountBalancesByType(
            organizationId,
            'ASSET',
            asOfDate
          ),
      ...
      // Categorize assets
      const currentAssets = assetBalances.filter(
        account =>
          account.accountCode.startsWith('1') &&
          parseInt(account.accountCode) < 1500
      );
      const nonCurrentAssets = assetBalances.filter(
        account =>
          account.accountCode.startsWith('1') &&
          parseInt(account.accountCode) >= 1500
      );
      ...
      const currentLiabilities = liabilityBalances.filter(
        account =>
          account.accountCode.startsWith('2') &&
          parseInt(account.accountCode) < 2500
      );
      const nonCurrentLiabilities = liabilityBalances.filter(
        account =>
          account.accountCode.startsWith('2') &&
          parseInt(account.accountCode) >= 2500
      );
      ...
      this.logger.log(
        `Generated balance sheet. Assets: ${totalAssets}, Liabilities + Equity: ${totalLiabilitiesAndEquity}, Balanced: ${balanceSheet.isBalanced}`
      );
      return balanceSheet;
    } catch (error) {
      this.logger.error(
  ...
  async generateIncomeStatement(
    ...
    try {
      this.logger.log(
        `Generating income statement for organization: ${organizationId} from ${periodFrom.toISOString()} to ${periodTo.toISOString()}`
      );
      ...
      // Categorize revenue
      const operatingRevenue = revenueBalances.filter(
        account =>
          account.accountCode.startsWith('41') ||
          account.accountCode.startsWith('42')
      );
      const nonOperatingRevenue = revenueBalances.filter(
        account =>
          account.accountCode.startsWith('43') ||
          parseInt(account.accountCode) >= 4400
      );
      ...
      const nonOperatingExpenses = expenseBalances.filter(
        account =>
          account.accountCode.startsWith('7') ||
          account.accountCode.startsWith('8') ||
          account.accountCode.startsWith('9')
      );
      ...
      const totalExpenses =
        totalCOGS + totalOperatingExpenses + totalNonOperatingExpenses;
      ...
      this.logger.log(
        `Generated income statement. Revenue: ${totalRevenue}, Expenses: ${totalExpenses}, Net Income: ${netIncome}`
      );
  ...
  async getAccountBalancesByType(
    ...
    try {
      return await this.generalLedgerRepository.getAccountBalancesByType(
        organizationId,
        accountType,
        asOfDate
      );
  ...
  async getCashFlowData(
    ...
    try {
      this.logger.log(
        `Getting cash flow data for organization: ${organizationId} from ${periodFrom.toISOString()} to ${periodTo.toISOString()}`,
      );
      ...
      this.logger.error(
        `Failed to get cash flow data: ${(error as Error).message}`,
  ...
  async validateGeneralLedgerIntegrity(organizationId: string): Promise<{
    ...
    try {
      this.logger.log(
        `Validating general ledger integrity for organization: ${organizationId}`
      );
      ...
      if (!trialBalance.isBalanced) {
        errors.push(
          `Trial balance is not balanced. Debits: ${trialBalance.totalDebits}, Credits: ${trialBalance.totalCredits}`
        );
      }
      ...
      for (const account of trialBalance.accounts) {
        ...
        if (account.normalBalance === 'CREDIT' && account.balance < 0) {
          warnings.push(
            `Credit account ${account.accountCode} has negative balance: ${account.balance}`
          );
        }
      }
      ...
      throw new BadRequestException(
        'Failed to validate general ledger integrity'
      );
    }
  }
}
