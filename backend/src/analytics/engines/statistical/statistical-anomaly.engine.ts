...
export class StatisticalAnomalyEngine implements IAnomalyDetectionEngine {
  ...
  async detectAnomalies(
    ...
    try {
      ...
      const isolationAnomalies = this.detectIsolationAnomalies(
        timeSeriesData.values,
        sensitivity
      );
      ...
      const combinedAnomalies = this.combineAnomalies(
        [zScoreAnomalies, iqrAnomalies, isolationAnomalies, seasonalAnomalies],
  ...
  private detectSeasonalAnomalies(
    ...
    values.forEach((value, index) => {
      ...
      const seasonalStdDev = this.calculateSeasonalStdDev(
        values,
        seasonLength,
        seasonIndex
      );
   ...
   */
  private calculateSeasonalStdDev(
    values: number[],
    seasonLength: number,
    seasonIndex: number
  ): number {
  ...
  private detectAnomalyClusters(anomalies: AnomalyPoint[]): AnomalyPoint[][] {
    ...
    anomalies.forEach((anomaly, index) => {
      if (index === 0) {
        ...
        const timeDiff =
          anomaly.timestamp.getTime() -
          anomalies[index - 1].timestamp.getTime();
   ...
   */
  private detectPeriodicAnomalies(
    ...
    for (let i = 1; i < anomalies.length; i++) {
      const diff =
        anomalies[i].timestamp.getTime() - anomalies[i - 1].timestamp.getTime();
      timeDiffs.push(diff);
    }
    ...
    if (coefficient < 0.3) {
      // Low variability indicates periodicity
  ...
  private generateAnomalyRecommendations(
    ...
    if (anomalies.length > 10) {
      recommendations.push(
        'High number of anomalies detected - review data quality and business processes'
      );
    }
    ...
    if (criticalAnomalies.length > 0) {
      recommendations.push(
        `${criticalAnomalies.length} critical anomalies require immediate attention`
      );
    }
    ...
    patterns.forEach(pattern => {
      switch (pattern.type) {
        case 'CLUSTERING':
          recommendations.push(
            'Clustered anomalies suggest systematic issues - investigate root causes'
          );
          break;
        case 'PERIODIC':
          recommendations.push(
            `Periodic anomalies every ${pattern.frequency} days - check for recurring events`
          );
          break;
      }
    });
    ...
    recommendations.push(
      'Monitor for mobile money transaction anomalies during peak periods'
    );
