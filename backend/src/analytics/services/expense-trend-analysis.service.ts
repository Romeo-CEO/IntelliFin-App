...
import { TimeSeriesData } from '../interfaces/analytics-engine.interface';
 ...
 * - Anomaly detection using Z-score and contextual analysis (now using Analytics Engine Factory)
...
export class ExpenseTrendAnalysisService {
  ...
  async analyzeExpenseTrends(
    ...
    try {
      ...
      this.logger.log(
        `Completed expense trend analysis for organization ${organizationId}`
      );
  ...
  /**
   * Detect expense anomalies using statistical analysis (updated to use factory)
   */
  async detectExpenseAnomalies(
    ...
    try {
      this.logger.log(
        `Detecting expense anomalies for organization ${organizationId} (using factory)`
      );
      ...
      // Convert expense data to AnalyticsData format for the engine
      const analyticsData: AnalyticsData = this.convertToAnalyticsData(
        financialData.expenses,
        organizationId,
        dateRange
      );

      // Get appropriate anomaly detection engine from the factory
      // For now, we'll assume statistical is preferred for anomaly detection in this service
      const anomalyEngine = this.engineFactory.getAnomalyDetectionEngine(
        {
          size: analyticsData.timeSeries?.values.length || 0,
          dimensions: 1,
          hasSeasonality: true,
        }, // Provide data characteristics
        false // Prefer statistical for now
      );

      // Detect anomalies using the engine
      const anomalyResult = await anomalyEngine.detectAnomalies(
        analyticsData,
        sensitivityLevel
      );

      // Map the anomaly results back to the service's expected format (ExpenseAnomaly[])
      // This is a simplified mapping; actual mapping may need more detail based on AnomalyResult structure
      const anomalies: ExpenseAnomaly[] = anomalyResult.anomalies.map(
        anomaly => ({
          id: `anomaly-${Date.now()}-${Math.random()}`, // Generate a unique ID
          organizationId,
          date: anomaly.timestamp, // Assuming anomaly result includes timestamp
          category: 'UNKNOWN', // Anomaly engine might not provide category, may need mapping
          description: `Detected anomaly with value ${anomaly.value} (Severity: ${anomaly.severity})`, // Example description
          severity: anomaly.severity, // Assuming severity is available
          details: anomaly, // Store full anomaly details if needed
        })
      );

      // Generate summary based on the anomaly result
      const summary = this.generateAnomalySummary(anomalies); // Reuse existing summary generation

      // Generate recommendations based on the anomaly result
      const recommendations = anomalyResult.recommendations || []; // Use recommendations from the engine

      this.logger.log(
        `Detected ${anomalies.length} expense anomalies using factory engine`
      );
  ...
  async generateCostOptimizationRecommendations(
    ...
    try {
      this.logger.log(
        `Generating cost optimization recommendations for organization ${organizationId}`
      );
      ...
      this.logger.error(
        `Failed to generate cost optimization recommendations: ${error.message}`,
        error.stack
      );
      throw error;
    }
  ...
  private calculateVolatility(amounts: number[]): number {
    ...
    const stdDev =
      this.baseAnalyticsService.calculateStandardDeviation(amounts);
   ...
   */
  private detectSeasonality(expenses: any[]): {
    detected: boolean;
    strength: number;
  } {
    ...
    expenses.forEach(expense => {
      const season = this.baseAnalyticsService.getZambianSeason(
        expense.expenseDate
      );
    ...
    const seasonalAverages = Object.entries(seasonalGroups).map(
      ([season, amounts]) => {
    ...
    const overallAverage =
      seasonalAverages.reduce((sum, val) => sum + val, 0) /
      seasonalAverages.length;
    const seasonalVariance =
      seasonalAverages.reduce(
        (sum, val) => sum + Math.pow(val - overallAverage, 2),
        0
      ) / seasonalAverages.length;

    const strength =
      overallAverage > 0 ? Math.sqrt(seasonalVariance) / overallAverage : 0;
  ...
  private generatePatternRecommendations(
    ...
    switch (pattern.direction) {
      case 'INCREASING':
        recommendations.push(
          `${category} expenses are increasing by ${pattern.changeRate.toFixed(1)}% - review for cost control opportunities`
        );
        recommendations.push(
          `Consider negotiating better rates with ${category} suppliers`
        );
        break;
      case 'DECREASING':
        recommendations.push(
          `${category} expenses are decreasing - maintain current cost management strategies`
        );
        break;
      case 'VOLATILE':
        recommendations.push(
          `${category} expenses show high volatility - implement better budgeting and approval controls`
        );
        recommendations.push(
          `Consider setting spending limits for ${category}`
        );
        break;
      case 'STABLE':
        recommendations.push(
          `${category} expenses are stable - good cost control`
        );
        break;
    }
    ...
    if (seasonality.detected) {
      recommendations.push(
        `${category} shows seasonal patterns - plan budget accordingly`
      );
    }
  ...
  private generateExpenseInsights(
    ...
    if (summary.overallGrowthRate > 20) {
      insights.push({
        ...
        recommendation:
          'Implement cost control measures and review major expense categories',
        priority: 'HIGH',
      });
    ...
    if (summary.volatileCategories > 0) {
      insights.push({
        ...
        recommendation:
          'Implement better budgeting and approval controls for volatile categories',
        priority: 'MEDIUM',
      });
    }
    ...
    if (summary.averageTaxDeductiblePercentage < 60) {
      insights.push({
        ...
        recommendation:
          'Review expense categorization and documentation for tax optimization',
        priority: 'MEDIUM',
      });
    }
  ...
  private generateOptimizationStrategies(
    ...
    const highGrowthCategories = trendAnalysis.patterns
      .filter(
        (p: ExpensePattern) => p.pattern === 'INCREASING' && p.changeRate > 15
      )
      .slice(0, 3);
  ...
  private generateAnomalyRecommendations(
    ...
    if (anomalies.length > 10) {
      recommendations.push(
        'Implement automated expense approval workflows to prevent anomalies'
      );
    }

    if (anomalies.length > 5) {
      recommendations.push(
        'Set up spending limits by category to control unusual expenses'
      );
    ...
    if (recommendations.length === 0) {
      recommendations.push(
        'Expense patterns are within normal ranges - maintain current controls'
      );
    }
  ...
  private generateAnomalySummary(anomalies: ExpenseAnomaly[]): any {
    const totalAnomalyAmount = anomalies.reduce(
      (sum, anomaly) => sum + Math.abs(anomaly.variance),
      0
    );

    const highSeverityCount = anomalies.filter(a => a.anomalyScore > 3).length;
    const mediumSeverityCount = anomalies.filter(
      a => a.anomalyScore > 2 && a.anomalyScore <= 3
    ).length;

    const categoryBreakdown: Record<string, number> = {};
    anomalies.forEach(anomaly => {
      categoryBreakdown[anomaly.category] =
        (categoryBreakdown[anomaly.category] || 0) + 1;
    });
  ...
  // Add a helper method to convert expense data to AnalyticsData format
  private convertToAnalyticsData(
    expenses: any[],
    organizationId: string,
    dateRange: DateRange
  ): AnalyticsData {
    // Assuming expenses is an array of objects with at least 'expenseDate' and 'amount'
    const timeSeriesData: TimeSeriesData = {
      // Use the imported TimeSeriesData
      values: expenses.map(e => e.amount),
      timestamps: expenses.map(e => new Date(e.expenseDate)),
      metadata: {
        organizationId,
        dataType: 'EXPENSE',
        dateRange,
