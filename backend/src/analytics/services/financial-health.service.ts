...
export class FinancialHealthService {
  ...
  private readonly SME_BENCHMARKS = {
    ...
    growthRate: 0.2, // 20% annual growth
    debtToEquity: 0.4, // 40% debt-to-equity ratio
  ...
  async calculateBusinessHealthScore(
    ...
    try {
      ...
      const components = await this.calculateHealthComponents(
        organizationId,
        startDate,
        endDate
      );
      ...
      return {
        ...
        components: {
          ...
          profitability: {
            score: components.profitability.score,
            weight: 0.25,
          },
          growth: { score: components.growth.score, weight: 0.2 },
  ...
  async getHealthScoreComponents(
    ...
    try {
      this.logger.log(
        `Getting detailed health score components for organization: ${organizationId}`
      );
  ...
  private async calculateHealthComponents(
    ...
    const growth = await this.calculateGrowthScore(
      organizationId,
      startDate,
      endDate,
      financialData
  ...
  private async getFinancialData(
    ...
    const [revenue, expenses, assets, liabilities, customers, invoices] =
      await Promise.all([
  ...
  private calculateCashFlowScore(
    financialData: any
  ): HealthScoreComponents['cashFlow'] {
    const operatingCashFlow =
      financialData.totalRevenue - financialData.totalExpenses;
    const cashFlowRatio =
      financialData.totalRevenue > 0
        ? operatingCashFlow / financialData.totalRevenue
        : 0;
    ...
    if (cashReserves > burnRate * 3)
      score += 20; // 3 months runway
    else if (cashReserves > burnRate) score += 10;
    else score -= 15;

    const insights: string[] = [];
    if (operatingCashFlow < 0)
      insights.push('Negative cash flow requires immediate attention');
    if (cashReserves < burnRate * 2)
      insights.push('Low cash reserves - consider improving collections');
  ...
  private calculateProfitabilityScore(
    financialData: any
  ): HealthScoreComponents['profitability'] {
    const grossProfit =
      financialData.totalRevenue - financialData.totalExpenses * 0.6; // Assume 60% COGS
    const netProfit = financialData.totalRevenue - financialData.totalExpenses;

    const grossMargin =
      financialData.totalRevenue > 0
        ? (grossProfit / financialData.totalRevenue) * 100
        : 0;
    const netMargin =
      financialData.totalRevenue > 0
        ? (netProfit / financialData.totalRevenue) * 100
        : 0;
    ...
    if (netMargin < 5)
      insights.push('Low profit margins - review pricing and costs');
    if (grossMargin < 25)
      insights.push('Low gross margins - optimize cost of goods sold');
  ...
  private async calculateGrowthScore(
    ...
    const prevData = await this.getFinancialData(
      organizationId,
      prevStartDate,
      prevEndDate
    );
    ...
    const revenueGrowth =
      prevData.totalRevenue > 0
        ? ((financialData.totalRevenue - prevData.totalRevenue) /
            prevData.totalRevenue) *
          100
        : 0;

    const customerGrowth =
      prevData.customerCount > 0
        ? ((financialData.customerCount - prevData.customerCount) /
            prevData.customerCount) *
          100
        : 0;
    ...
    const insights: string[] = [];
    if (revenueGrowth < 0)
      insights.push('Revenue is declining - focus on growth strategies');
    if (customerGrowth < 0)
      insights.push('Customer base is shrinking - improve retention');
  ...
  private calculateEfficiencyScore(
    financialData: any
  ): HealthScoreComponents['efficiency'] {
    const assetTurnover =
    ...
    const revenuePerCustomer =
      financialData.customerCount > 0
        ? financialData.totalRevenue / financialData.customerCount
        : 0;
    ...
    const insights: string[] = [];
    if (costRatio > 90)
      insights.push('High cost ratio - focus on operational efficiency');
    if (assetTurnover < 0.5)
      insights.push('Low asset utilization - optimize asset usage');
    if (revenuePerCustomer < 1000)
      insights.push('Low revenue per customer - improve customer value');
  ...
  private async calculateStabilityScore(
    ...
    financialData.invoices.forEach((invoice: any) => {
      ...
      customerRevenue.set(
        invoice.customerId,
        existing + (invoice.paidAmount?.toNumber() || 0)
      );
    });

    const revenueArray = Array.from(customerRevenue.values()).sort(
      (a, b) => b - a
    );
    const topCustomerRevenue = revenueArray[0] || 0;
    const customerConcentration =
      financialData.totalRevenue > 0
        ? (topCustomerRevenue / financialData.totalRevenue) * 100
        : 0;
    ...
    if (customerConcentration > 50)
      insights.push(
        'High customer concentration risk - diversify customer base'
      );
    if (debtToEquity > 1)
      insights.push('High debt levels - focus on debt reduction');
    if (financialData.customerCount < 5)
      insights.push('Limited customer base - expand market reach');
   ...
   */
  private determineHealthCategory(
    score: number
  ): BusinessHealthScore['category'] {
  ...
  private generateHealthRecommendations(
    ...
    if (overallScore < 40) {
      recommendations.push(
        'Business health is critical - consider professional financial consultation'
      );
    } else if (overallScore < 70) {
      recommendations.push('Focus on improving cash flow and profitability');
    } else {
      recommendations.push(
        'Maintain current performance and explore growth opportunities'
      );
    }
  ...
  private async getBenchmarks(
    organizationId: string
  ): Promise<BusinessHealthScore['benchmarks']> {
    ...
    return {
      ...
      quickRatio:
        currentLiabilities > 0
          ? (currentAssets - currentAssets * 0.3) / currentLiabilities
          : 0,
  ...
  private calculateProfitabilityRatios(
    ...
    return {
      ...
      returnOnAssets:
        financialData.totalAssets > 0
          ? (netProfit / financialData.totalAssets) * 100
          : 0,
    ...
    return {
      assetTurnover:
        financialData.totalAssets > 0
          ...
          : 0,
      receivablesTurnover:
        receivables > 0 ? financialData.totalRevenue / receivables : 0,
  ...
  private calculateLeverageRatios(
    ...
    return {
      debtToEquity:
        equity > 0 ? (financialData.totalLiabilities / equity) * 100 : 0,
      debtToAssets:
        financialData.totalAssets > 0
