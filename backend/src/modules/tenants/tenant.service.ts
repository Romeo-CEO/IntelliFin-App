...
export class TenantService {
  ...
  public async createTenant(createTenantDto: CreateTenantDto) {
    const { name, slug, businessType, zraTin, email, phone, address, city, industry } = createTenantDto;
    ...
    try {
      ...
      if (existingTenant) {
        ...
        if (existingTenant.zraTin === zraTin) {
          throw new ConflictException('Tenant with this ZRA TIN already exists');
        }
      }
      ...
      this.logger.warn(`⚠️  Tenant schema creation skipped for testing: ${tenant.id}`);
   ...
   */
  public async updateTenant(tenantId: string, updateTenantDto: UpdateTenantDto) {
  ...
  public async suspendTenant(tenantId: string, reason?: string) {
    ...
    this.logger.warn(`Suspended tenant: ${tenant.name} (${tenantId}). Reason: ${reason}`);
  ...
  private async initializeTenantSchema(tenantId: string) {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
      const errorStack = error instanceof Error ? error.stack : undefined;
      this.logger.error(`Failed to initialize tenant schema: ${errorMessage}`, errorStack);
      throw error;
    } finally {
  ...
  private async createDefaultCategories(tenantClient: PrismaClient) {
    const defaultCategories = [
      // Income categories
      { name: 'Sales Revenue', type: 'INCOME', chartOfAccountsCode: '4000', isSystem: true },
      { name: 'Service Revenue', type: 'INCOME', chartOfAccountsCode: '4100', isSystem: true },
      { name: 'Interest Income', type: 'INCOME', chartOfAccountsCode: '4200', isSystem: true },
      { name: 'Other Income', type: 'INCOME', chartOfAccountsCode: '4900', isSystem: true },

      // Expense categories
      { name: 'Cost of Goods Sold', type: 'EXPENSE', chartOfAccountsCode: '5000', isSystem: true },
      { name: 'Office Supplies', type: 'EXPENSE', chartOfAccountsCode: '6100', isSystem: true },
      { name: 'Rent Expense', type: 'EXPENSE', chartOfAccountsCode: '6200', isSystem: true },
      { name: 'Utilities', type: 'EXPENSE', chartOfAccountsCode: '6300', isSystem: true },
      { name: 'Telephone & Internet', type: 'EXPENSE', chartOfAccountsCode: '6400', isSystem: true },
      { name: 'Vehicle Expenses', type: 'EXPENSE', chartOfAccountsCode: '6500', isSystem: true },
      { name: 'Travel & Entertainment', type: 'EXPENSE', chartOfAccountsCode: '6600', isSystem: true },
      { name: 'Professional Fees', type: 'EXPENSE', chartOfAccountsCode: '6700', isSystem: true },
      { name: 'Insurance', type: 'EXPENSE', chartOfAccountsCode: '6800', isSystem: true },
      { name: 'Bank Charges', type: 'EXPENSE', chartOfAccountsCode: '6900', isSystem: true },

      // Asset categories
      { name: 'Cash and Bank', type: 'ASSET', chartOfAccountsCode: '1000', isSystem: true },
      { name: 'Accounts Receivable', type: 'ASSET', chartOfAccountsCode: '1200', isSystem: true },
      { name: 'Inventory', type: 'ASSET', chartOfAccountsCode: '1300', isSystem: true },
      { name: 'Equipment', type: 'ASSET', chartOfAccountsCode: '1500', isSystem: true },

      // Liability categories
      { name: 'Accounts Payable', type: 'LIABILITY', chartOfAccountsCode: '2000', isSystem: true },
      { name: 'VAT Payable', type: 'LIABILITY', chartOfAccountsCode: '2100', isSystem: true },
      { name: 'PAYE Payable', type: 'LIABILITY', chartOfAccountsCode: '2200', isSystem: true },

      // Equity categories
      { name: 'Owner\'s Equity', type: 'EQUITY', chartOfAccountsCode: '3000', isSystem: true },
      { name: 'Retained Earnings', type: 'EQUITY', chartOfAccountsCode: '3100', isSystem: true },
    ];
