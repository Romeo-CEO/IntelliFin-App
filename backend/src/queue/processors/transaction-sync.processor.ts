...
import { QueueService, SyncAccountJobData, SyncAllAccountsJobData, UpdateBalanceJobData, RetryFailedSyncJobData } from '../queue.service';
...
export class TransactionSyncProcessor {
  ...
  async processSyncAccountTransactions(job: Job<SyncAccountJobData>) {
    const { accountId, organizationId, startDate, endDate, isManual, userId } = job.data;
    
    this.logger.log(`Processing sync for account: ${accountId} (Job: ${job.id})`);
    ...
    try {
      ...
      const result = await this.transactionSyncService.syncAccountTransactions(accountId, {
  ...
  async processSyncAllAccounts(job: Job<SyncAllAccountsJobData>) {
    const { organizationId, isManual, userId } = job.data;
    
    this.logger.log(`Processing sync for all accounts in organization: ${organizationId || 'all'} (Job: ${job.id})`);

    try {
      let results;
      
      if (organizationId) {
        results = await this.transactionSyncService.syncAllAccountsForOrganization(organizationId);
      } else {
  ...
  async processUpdateAccountBalance(job: Job<UpdateBalanceJobData>) {
    ...
    this.logger.log(`Processing balance update for account: ${accountId} (Job: ${job.id})`);
    ...
    try {
      ...
      const balanceInfo = await this.airtelApiClient.getAccountBalance(tokens.accessToken);
      ...
      this.logger.log(`Updated balance for account ${accountId}: ${balanceInfo.balance} ${balanceInfo.currency}`);
      ...
      this.logger.error(`Failed to update balance for account ${accountId}:`, error);
      throw error;
    }
  }
  ...
  async processRetryFailedSync(job: Job<RetryFailedSyncJobData>) {
    ...
    this.logger.log(`Processing retry for failed sync: ${originalJobId}, attempt: ${retryCount} (Job: ${job.id})`);
    ...
    try {
      ...
      this.logger.error(`Failed to schedule retry for job ${originalJobId}:`, error);
      throw error;
    }
  }
  ...
  async onFailed(job: Job, error: Error) {
    ...
    const retryDecision = this.retryStrategy.shouldRetryJob(job, error, retryConfig);
    
    this.retryStrategy.logRetryDecision(job, error, retryDecision);

    // Handle specific retry logic for sync jobs
    if (job.name === JOB_TYPES.SYNC_ACCOUNT_TRANSACTIONS && !retryDecision.shouldRetry) {
  ...
  private async syncAllAccountsGlobally() {
    ...
    for (const org of organizations) {
      try {
        const results = await this.transactionSyncService.syncAllAccountsForOrganization(org.id);
        allResults.push(...results);
      } catch (error) {
        this.logger.error(`Failed to sync accounts for organization ${org.id}:`, error);
      }
    }
