...
import { TaxObligationStatus, TaxObligationType } from '@prisma/client';
...
export class TaxObligationController {
  ...
  @ApiOperation({
    summary: 'Calculate penalties and interest for overdue obligations',
  })
  @ApiResponse({
    status: 200,
  ...
  async calculatePenalties(@Request() req: any) {
    try {
      await this.taxObligationService.calculatePenaltiesAndInterest(
        req.user.organizationId
      );
  ...
  @ApiResponse({
    status: 200,
    description: 'Tax obligations summary retrieved successfully',
  })
  ...
  @Get('upcoming')
  @ApiOperation({
    summary: 'Get upcoming tax obligations (due in next 30 days)',
  })
  @ApiResponse({
    status: 200,
    description: 'Upcoming obligations retrieved successfully',
  })
  async getUpcomingObligations(@Request() req: any) {
    try {
      const obligations =
        await this.taxObligationService.getUpcomingObligations(
          req.user.organizationId
        );
  ...
  @ApiResponse({
    status: 200,
    description: 'Overdue obligations retrieved successfully',
  })
  ...
  @ApiResponse({
    status: 200,
    description: 'Obligations by type retrieved successfully',
  })
  ...
  async getAnalytics(
    ...
    try {
      ...
      const analytics = {
        ...
        averagePaymentTime:
          this.calculateAveragePaymentTime(completedObligations),
        largestObligation: Math.max(
          ...obligations.map(o => o.amountDue.toNumber()),
          0
        ),
        totalTaxLiability: obligations.reduce(
          (sum, o) => sum + o.amountDue.toNumber(),
          0
        ),
      };
    ...
    const totalDays = completedObligations.reduce((sum, obligation) => {
      if (obligation.paidAt && obligation.dueDate) {
        const days = Math.ceil(
          (obligation.paidAt.getTime() - obligation.dueDate.getTime()) /
            (1000 * 60 * 60 * 24)
        );
