...
export interface WithholdingCertificateWithPeriod
  extends WithholdingTaxCertificate {
  taxPeriod: {
    taxType: string;
    periodStart: Date;
    periodEnd: Date;
...
export class WithholdingTaxService {
  ...
  async createCertificate(
    dto: CreateWithholdingCertificateDto
  ): Promise<WithholdingTaxCertificate> {
    try {
      this.logger.log(
        `Creating withholding tax certificate for ${dto.supplierName}`
      );
      ...
      const withholdingRate =
        dto.withholdingRate || this.getDefaultRate(dto.serviceType);
      ...
      this.logger.error(
        `Failed to create withholding certificate: ${error.message}`,
        error.stack
      );
      throw error;
    }
  ...
  async getCertificates(
    ...
    try {
      ...
      this.logger.error(
        `Failed to get withholding certificates: ${error.message}`,
        error.stack
      );
      throw error;
    }
    ...
    try {
      ...
      if (certificate.status !== WithholdingCertificateStatus.ISSUED) {
        throw new BadRequestException(
          'Only issued certificates can be submitted'
        );
      }
      ...
      this.logger.error(
        `Failed to submit certificate to ZRA: ${error.message}`,
        error.stack
      );
      throw error;
    }
  ...
  async bulkSubmitToZRA(
    ...
    try {
      ...
      this.logger.log(
        `Bulk submission completed: ${submitted} submitted, ${failed} failed`
      );
      return { submitted, failed, errors };
    } catch (error: any) {
      this.logger.error(
        `Failed to bulk submit certificates: ${error.message}`,
        error.stack
      );
      throw error;
    }
    ...
    try {
      ...
      summary.submissionRate =
        certificates.length > 0
          ? Math.round((submittedCount / certificates.length) * 100)
          : 100;

      return summary;
    } catch (error: any) {
      this.logger.error(
        `Failed to get withholding summary: ${error.message}`,
        error.stack
      );
      throw error;
    }
  ...
  async generateMonthlyReturn(
    ...
    try {
      ...
      const summary = await this.getWithholdingSummary(
        organizationId,
        year,
        month
      );
      ...
      this.logger.error(
        `Failed to generate monthly return: ${error.message}`,
        error.stack
      );
      throw error;
    }
