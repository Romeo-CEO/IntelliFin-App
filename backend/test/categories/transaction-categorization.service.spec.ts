...
import { Transaction, CategorizationConfidence, TransactionType } from '@prisma/client';
...
interface MockCategorizationRuleRepository extends CategorizationRuleRepository {
...
describe('TransactionCategorizationService', () => {
  ...
  beforeEach(async () => {
    ...
    ruleRepository = mockRepo as unknown as MockProxy<MockCategorizationRuleRepository>;
  });
  ...
  describe('categorizeTransaction', () => {
    it('should categorize transaction based on matching rules', async () => {
      // Arrange
      const transaction = createMockTransaction({ description: 'Test transaction with keyword' });
      const mockRule = createMockRule();
      
      // Mock the repository to return our test rule
      ruleRepository.findMatchingRules.mockResolvedValue([{
        ...mockRule,
        matchesTransaction: (tx: Transaction) => tx.description?.includes('keyword') || false
      }]);
      ...
      expect(ruleRepository.findMatchingRules).toHaveBeenCalledWith(transaction);
    ...
    it('should handle transactions with no matching rules', async () => {
      // Arrange
      const transaction = createMockTransaction({ description: 'Transaction with no matching rules' });
